...
class bSocial_Facebook_Comments
{
	...
	{
		if ( ! bsocial()->facebook()->oauth()->service->is_configured() )
		{
			return;
		} // END if

		$this->app_id = bsocial()->facebook()->oauth()->service->app_id;
		$this->secret = bsocial()->facebook()->oauth()->service->secret;
	...
	{
		add_action( 'bsocial_fb_ingest_comments', array( $this, 'ingest_comments' ) );
		add_action( 'wp_ajax_new_fb_comment', array( $this, 'do_ajax' ) );
	...
	public function fb_api_fetch( $url, $post_id )
	{
		...
		if ( is_wp_error( $response ) )
		{
			...
			wp_schedule_single_event( time() + 181, 'bsocial_fb_ingest_comments', array( $post_id ) );
			die;
		}
		else
		{
